...
<?php
	if ($language == "JP"){
	 echo '<p id="machine">テーマ名</p>';
	}
	else {
		echo '<p id="machine">TIÊU ĐỀ</p>';
	}
?>

<select id="select2" onClick="titleChange(this)">
    <option value=""></option>
</select>
...
<script>
	...
	function yearChange(sel) {		
  	var textYear = sel.options[sel.selectedIndex].text;
        ...
        xmlhttp.open("GET", "manuprocess.php?year=" + textYear, true);
        xmlhttp.send();
	}
    ...
    var textYear = select1.options[select1.selectedIndex].text;		
  	var textTitle = sel.options[sel.selectedIndex].text;
        ...
        xmlhttp.open("GET", "manuprocess.php?year=" + textYear + "&title=" + textTitle, true);
        xmlhttp.send();
	}
