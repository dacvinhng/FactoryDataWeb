...
if(isset($_REQUEST['ID'])){
	...
	$sql = "SELECT * FROM hub1850 WHERE position = '$position'";
...
<body>
		...
		<div class="viewer1" name="viewer1">
			<div id="showimage0">
				<img id="image0" src="images/hub1850.png" alt="housing">
			</div>
			<p id="line0">HUB 1850</p>
			<?php
			if ($language == "JP"){ 
  ...
  <script>
		...
		mybackBtn.addEventListener('click', function(event) {
			<?php
			if ($language == "JP"){ 
			echo 'window.location.href = "chonmau.php?language=JP&mauHang=HUB-04742-1850";';
			}
			else{
			echo 'window.location.href = "chonmau.php?mauHang=HUB-04742-1850";';
			}
			?>
		});
...
var myhomeBtn = document.getElementById('homeBtn');
	myhomeBtn.addEventListener('click', function(event) {
		...
		function changeVN(){
			window.location.href = "HUB1850.php";
		}

		function changeJP(){
			
			window.location.href = "HUB1850.php?language=JP";
		}
